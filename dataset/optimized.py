...
# ---------------- SMART DOWNLOAD (no geo-check) ----------------
def smart_download_no_geocheck(url, save_dir):
    name = url.split("/")[-1]
    final_path = os.path.join(save_dir, name)
    if os.path.exists(final_path):
        print("✔ Exists:", name)
...
for date in daterange(start, end):
    ...
    for u06, u21 in filtered_pairs:
        ok1 = smart_download(u06, SAVE_MOD06)            # geo-check on MOD06
        if not ok1:
            print("⏭️ Skipping MOD021KM — MOD06 failed for this timestamp")
            continue
        ok2 = smart_download_no_geocheck(u21, SAVE_MOD21) # no geo-check for MOD21
        if not ok2:
            print("⚠️ Pair incomplete — MOD021KM failed")                                            